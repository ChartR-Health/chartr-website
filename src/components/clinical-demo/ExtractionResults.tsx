...
import { CheckCircle, AlertTriangle, Info, ArrowRight, Target, Edit, Save, X, ChevronDown, ChevronRight, Brain, Flag } from 'lucide-react';
...
export function ExtractionResults({ onNavigate }: ExtractionResultsProps) {
  ...
  const [currentStep, setCurrentStep] = useState('Connecting to EPIC...');
  ...
  const [collapsedCategories, setCollapsedCategories] = useState<Set<string>>(
    new Set(['Patient Demographics & Risk Factors', 'Clinical Data'])
  ); // Start with all categories collapsed
  const [flaggedVariables, setFlaggedVariables] = useState<Set<string>>(new Set());
  const [sourceNoteRef, setSourceNoteRef] = useState<HTMLDivElement | null>(null);
  ...
  const getVariableCategory = (key: string) => {
    if (['sex', 'age', 'height', 'weight', 'bmi', 'diabetes', 'tobaccoUse', 'covid19'].includes(key)) return 'Patient Demographics & Risk Factors';
  ...
  const getSingleSourceContent = (source: string) => {
    ...
    switch (cleanSource) {
      ...
      case 'preop':
        return currentPatient.notes.preop || 'Pre-op note not available';
      default:
  ...
  const handleFlagVariable = (variableKey: string) => {
    const newFlagged = new Set(flaggedVariables);
    if (newFlagged.has(variableKey)) {
      newFlagged.delete(variableKey);
    } else {
      newFlagged.add(variableKey);
    }
    setFlaggedVariables(newFlagged);
  };

  const jumpToHighlight = () => {
    if (sourceNoteRef) {
      const highlighted = sourceNoteRef.querySelector('[style*="background-color: #fef08a"]');
      if (highlighted) {
        highlighted.scrollIntoView({ behavior: 'smooth', block: 'center' });
        // Flash effect
        const originalStyle = highlighted.getAttribute('style') || '';
        highlighted.setAttribute('style', originalStyle + '; animation: pulse 1s ease-in-out;');
        setTimeout(() => {
          highlighted.setAttribute('style', originalStyle);
        }, 1000);
      }
    }
  };
    ...
    switch (variableKey) {
      ...
      case 'diabetes':
        if (value === 'Yes') {
          ...
          patterns.push('PAST MEDICAL HISTORY:', 'Past Medical History:', 'PAST MEDICAL HISTORY', 'Past Medical History');
        }
        break;
      case 'efMethod':
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-medium text-blue-900">ChartR AI Analysis in Progress</h3>
                <p className="text-blue-700 mb-3">{currentStep}</p>
        ...
        <>
          ...
          <div className="mb-8">
            ...
            <div className="flex justify-center mb-6">
              <div className="inline-flex items-center bg-slate-800/60 backdrop-blur-xl border border-white/20 rounded-xl p-1 shadow-lg">
            ...
            <Card className="bg-slate-800/40 backdrop-blur-xl border border-white/10">
              <CardContent className="p-3">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-xs text-slate-400">Total Variables</p>
                    <p className="text-lg font-bold text-white">{totalVariables}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-3">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-white">{highConfidenceCount}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-slate-800/40 backdrop-blur-xl border border-white/10">
              <CardContent className="p-3">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-xs text-slate-400">Review Needed</p>
          ...
          {Object.values(currentExtractedData).some((data: any) => data.confidence < 90) && (
            <div className="mb-8 p-4 bg-amber-500/10 rounded-lg border border-amber-400/20 backdrop-blur-sm">
              <div className="flex items-center space-x-2">
                                ...
                                {data.dataType === 'structured' ? (
                                  <>
                                    ...
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={() => handleFlagVariable(key)}
                                      className={`${
                                        flaggedVariables.has(key)
                                          ? 'text-red-300 border-red-400/30 bg-red-500/20 hover:bg-red-500/30'
                                          : 'text-slate-300 border-slate-400/30 bg-slate-500/20 hover:bg-slate-500/30'
                                      }`}
                                      title={flaggedVariables.has(key) ? 'Flagged for provider review' : 'Flag for provider review'}
                                    >
              ...
              {viewingVariable && currentExtractedData[viewingVariable] && (
                <div className="space-y-4 overflow-y-auto max-h-[60vh] pr-2">
                  ...
                  {currentExtractedData[viewingVariable]?.dataType === 'structured' ? (
                    ...
                    <div 
                      ref={setSourceNoteRef}
                      className="bg-slate-50 p-4 rounded-lg border mt-2 max-h-96 overflow-y-auto"
                    >
                      <div 
                        className="whitespace-pre-wrap text-sm font-mono text-slate-700 leading-relaxed"
                        dangerouslySetInnerHTML={{
