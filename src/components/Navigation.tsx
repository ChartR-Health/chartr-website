...
const Navigation = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 chartr-bg-accent border-b border-indigo-400/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navigationItems.map((item) => (
              <div key={item.name} className="relative group">
                {item.submenu ? (
                  <>
                    <button className="flex items-center space-x-1 text-slate-300 hover:text-white transition-all duration-300 chartr-body-medium px-3 py-2 rounded-lg hover:bg-white/5">
                      <span>{item.name}</span>
                    ...
                    <div className="absolute top-full right-0 w-80 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 mt-2 z-50">
                      <div className="chartr-bg-accent bg-slate-900 shadow-2xl border border-white/10 rounded-2xl p-6">
                  ...
                  <Link
                    href={item.href}
                    className="text-slate-300 hover:text-white transition-all duration-300 chartr-body-medium px-3 py-2 rounded-lg hover:bg-white/5"
                  >
                    {item.name}
                  </Link>
                )}
        ...
        {isOpen && (
          <div className="md:hidden">
            <div className="px-4 py-6 space-y-4 chartr-bg-accent bg-slate-900 shadow-xl">
              {navigationItems.map((item) => (
                <div key={item.name}>
                  {item.submenu ? (
                    <>
